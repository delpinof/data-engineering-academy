...
object AlimazonC3v3 {
    ...
    def main(args: Array[String]) {
        ...
        def runProductReports(i:Int) : Unit = {
            ...
            productWeek.cache()
            saveToFile(productWeek.orderBy($"gross_sales".desc).limit(10),s"top10_products_by_gross_sales_$i")
            saveToFile(productWeek.orderBy($"orders_count".desc).limit(10),s"top10_products_by_orders_$i")
        ...
        def runCustomerReports(i:Int) : Unit = {
            ...
            customerMonth.cache()
            saveToFile(customerMonth.orderBy($"gross_sales".desc).limit(10),s"top10_customers_by_gross_spending_$i")
            saveToFile(customerMonth.orderBy($"orders_count".desc).limit(10),s"top10_customers_by_orders_$i")
